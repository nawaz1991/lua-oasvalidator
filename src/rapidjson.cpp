...
static const luaL_Reg methods[] = {
    ...
    { "init", Userdata<OASValidator>::create },
...
{
	lua_newtable(L); // [oasvalidator]

	luax::setfuncs(L, methods); // [oasvalidator]

	lua_pushliteral(L, "oasvalidator"); // [oasvalidator, name]
	lua_setfield(L, -2, "_NAME"); // [oasvalidator]

	lua_pushliteral(L, LUA_RAPIDJSON_VERSION); // [oasvalidator, version]
	lua_setfield(L, -2, "_VERSION"); // [oasvalidator]

    values::push_null(L); // [oasvalidator, json.null]
    lua_setfield(L, -2, "null"); // [oasvalidator]
	...
	Userdata<OASValidator>::luaopen(L);
